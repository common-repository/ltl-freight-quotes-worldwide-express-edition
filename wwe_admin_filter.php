 ...
 */
function ltl_freight_woocommrec_avaibility_error()
{
 ...
 * @param $settings
 */
function ltl_shipping_sections($settings)
{
 ...
 */
function ltl_check_woo_version()
{

    $wcPluginVersion = new ltl_shipping_get_quotes();
    $woo_version = $wcPluginVersion->ltl_get_woo_version_number();

    $version = '2.6';
    if (!version_compare($woo_version["woocommerce_plugin_version"], $version, ">=")) {

        add_action('admin_notices', 'ltl_admin_notice_failure');
    }
}
 ...
 * Admin Notices
 */
function ltl_admin_notice_failure()
{
    ?>
 ...
 */
function ltl_hide_shipping_based_on_class($available_methods)
{
    ...
    if (get_option('wc_pervent_proceed_checkout_eniture') == 'backup_rates' && count($available_methods) > 1) {
        $backup_rate_id = 'ltl_shipping_method:backup_rates';
    ...
    if (get_option('wc_settings_wwe_allow_other_plugins') == 'no'
        ...
        foreach ($available_methods as $value) {
            if ($value->method_id == 'ltl_shipping_method') {
 ...
 * @param  $post_id
 */
function ltl_save_carrier_status($post_id)
{

    if (isset($_POST['action']) && $_POST['action'] == 'save_carrier_status') {

        global $wpdb;
        $all_freight_array = [];
        $count_carrier = 1;
        $ltl_freight_get = $wpdb->get_results('SELECT * FROM ' . WWE_CARRIERS . ' order by speed_freight_carrierName ASC');
        foreach ($ltl_freight_get as $ltl_freight_get_value):
            ...
            if (isset($_POST[$ltl_freight_get_value->speed_freight_carrierSCAC . $ltl_freight_get_value->id]) && $_POST[$ltl_freight_get_value->speed_freight_carrierSCAC . $ltl_freight_get_value->id] == 'on') {
                $en_c_status = 1;
                $wpdb->query($wpdb->prepare("UPDATE " . WWE_CARRIERS . " SET carrier_status = %d  WHERE speed_freight_carrierSCAC = '$ltl_freight_get_value->speed_freight_carrierSCAC'", $en_c_status));
            } else {
                $c_status = 0;
                $wpdb->query($wpdb->prepare("UPDATE " . WWE_CARRIERS . " SET carrier_status = %d WHERE speed_freight_carrierSCAC = '$ltl_freight_get_value->speed_freight_carrierSCAC' ", $c_status));
 ...
 * @param $methods
 */
function ltl_add_LTL_shipping_method($methods)
{
 ...
 * @return string
 */
function ltl_remove_free_label($full_label, $method)
{
