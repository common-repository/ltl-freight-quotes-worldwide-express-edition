...
/**
 * Class group_ltl_shipments
 */
class group_ltl_shipments extends Wwe_Ltl_Liftgate_As_Option
{
    // Crowler work
    ...
    public $origin = [];
    public $prevent_resi_available_in_cart = false;
    ...
    function ltl_package_shipments($package, $ltl_res_inst, $freight_zipcode)
    {
        ...
        $this->destination_Address_wwe_lfq = $ltl_shipping_get_quotes->destinationAddressWwe();
        ...
        $en_prevent_resi_addon = apply_filters('en_prevent_resi_custom_addon', false);
        $count = 0;
        ...
        foreach ($package['contents'] as $item_id => $values) {
            ...
            $product_level_markup = $this->wwe_ltl_get_product_level_markup($_product, $values['variation_id'], $values['product_id'], $values['quantity']);

            // get product class
            $freightClass_ltl_gross = $this->wwe_ltl_freight_class($values, $_product);
            ...
            if (!empty($origin_address)) {
                ...
                $hm_plan = apply_filters('wwe_quests_quotes_plans_suscription_and_features', 'hazardous_material');
                ...
                if (!$_product->is_virtual() && !in_array($en_ship_class, $en_get_current_classes_arr)) {
                    ...
                    if (!empty($product_tags)) {
                        ...
                        if (isset($ltl_package[$ship_type][$locationId]['product_tags'])) {
                            $ltl_package[$ship_type][$locationId]['product_tags'] = array_merge($ltl_package[$ship_type][$locationId]['product_tags'], $product_tag_names);
                        } else {
                            $ltl_package[$ship_type][$locationId]['product_tags'] = $product_tag_names;
                        }
                    } else {
                        $ltl_package[$ship_type][$locationId]['product_tags'] = [];
                    }

                    // Product quantity
                    if (isset($ltl_package[$ship_type][$locationId]['product_quantities'])) {
                        $ltl_package[$ship_type][$locationId]['product_quantities'] += floatval($values['quantity']);
                    } else {
                        $ltl_package[$ship_type][$locationId]['product_quantities'] = floatval($values['quantity']);
                    }

                    // Product price
                    if (isset($ltl_package[$ship_type][$locationId]['product_prices'])) {
                        $ltl_package[$ship_type][$locationId]['product_prices'] += (floatval($_product->get_price()) * floatval($values['quantity']));
                    } else {
                        $ltl_package[$ship_type][$locationId]['product_prices'] = (floatval($_product->get_price()) * floatval($values['quantity']));
                    }
                    ...
                    // custom work
                    if($en_prevent_resi_addon){
                        $en_resi_and_packaging_fee_values = apply_filters('en_get_prevent_resi_packaging_fee_values', $values, $_product);
                        $ltl_package[$ship_type][$locationId]['prevent_resi_value'][$count] = $en_resi_and_packaging_fee_values['prevent_resi_value'];
                        $ltl_package[$ship_type][$locationId]['packaging_fee_value'][$count] = $en_resi_and_packaging_fee_values['packaging_fee_value'];
                        if(!empty($en_resi_and_packaging_fee_values['prevent_resi_available_in_cart']) && $en_resi_and_packaging_fee_values['prevent_resi_available_in_cart'] == 'yes'){
                            $this->prevent_resi_available_in_cart = TRUE;
                        }
                    }
        ...
        if (count($pricing_product_origins) > 0) {
            $ltl_package['pricing_product_origins'] = $pricing_product_origins;
        }
    ...
    function wwe_ltl_get_origin($_product, $values, $ltl_res_inst, $ltl_zipcode)
    {
        ...
        if ($enable_dropship == 'yes') {
            ...
            // Multi Dropship
            $multi_dropship = apply_filters('wwe_quests_quotes_plans_suscription_and_features', 'multi_dropship');
        ...
        if (empty($locations_list)) {
            // Multi Warehouse
            $multi_warehouse = apply_filters('wwe_quests_quotes_plans_suscription_and_features', 'multi_warehouse');
     ...
     * @return array
     */
    function wwe_ltl_freight_class($values, $_product)
    {
    ...
    */
    function wwe_ltl_get_product_level_markup($_product, $variation_id, $product_id, $quantity)
    {
